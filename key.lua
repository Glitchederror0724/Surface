...
local Config = {
    ...
    -- [7] Hub Info
    HubName         = "Surface",
    HubDescription  = "Surface is a script used to get assect to basic cmds anywere and has a built-in script loader"
}
...
-- Safe request function (supports multiple executors)
...
-------------------------------------------------------------------------------
--! MAIN SCRIPT LOADER (FIXED with robust error handling)
-------------------------------------------------------------------------------
local function StartMainScript()
    ...
    -- Expose common functions globally to prevent "attempt to call nil value"
    ...
    -- Download the main script with fallback methods
    local content, err
    local downloadSuccess, downloadResult = pcall(function()
        -- Try game:HttpGet first (standard Roblox)
    ...
    if downloadSuccess then
        ...
        -- Fallback to safeRequest (supports syn_request, http_request, etc.)
        local response, reqErr = safeRequest({Url = Config.MainScriptURL, Method = "GET"})
        if response and response.StatusCode == 200 then
            content = response.Body
        else
            error("Failed to download main script. game:HttpGet error: " .. tostring(downloadResult) .. 
                  " | safeRequest error: " .. tostring(reqErr))
        end
    end
    
    -- Attempt to compile the script
    local scriptFunc, loadErr = loadstring(content)
    if not scriptFunc then
        -- Show a preview of the content to help diagnose (e.g., if it's HTML)
        local preview = string.sub(content, 1, 300)
        error(string.format("Failed to compile main script: %s\nContent preview:\n%s", 
              tostring(loadErr), preview))
    end
    
    -- Execute the script with error catching
    local execSuccess, execErr = pcall(scriptFunc)
    if not execSuccess then
        error("Main script execution error: " .. tostring(execErr))
    end
end

-------------------------------------------------------------------------------
--! GUI CREATION (unchanged, but included for completeness)
-------------------------------------------------------------------------------
